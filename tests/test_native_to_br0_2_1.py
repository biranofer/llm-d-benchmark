...
*_rate, request_size_bytes->request_size), and preserves the v0.2 content it
inherits from the reused v0.2 converter.
"""
...
def test_image_stats_mapped(report, native):
    ...
    _assert_maps(img.bytes, raw["bytes"], Units.BYTES)
...
def test_video_stats_mapped(report, native):
    ...
    _assert_maps(vid.bytes, raw["bytes"], Units.BYTES)
    ...
    # invent one.
    assert not hasattr(vid, "seconds")
...
def test_audio_stats_mapped(report, native):
    ...
    _assert_maps(aud.seconds, raw["seconds"], Units.S)
    _assert_maps(aud.bytes, raw["bytes"], Units.BYTES)
...
def test_inherited_v0_2_content_preserved(report, native):
    ...
    assert agg.throughput.request_rate.mean == native["successes"]["throughput"]["requests_per_sec"]
    assert agg.throughput.output_token_rate.mean == (
